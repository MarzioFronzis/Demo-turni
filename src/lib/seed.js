...
export async function seedEsempio(lunediSettimanaVista) {
  ...
  const skills = [
    { ref: doc(collection(db, "skills")), nome: "cassa" },
    { ref: doc(collection(db, "skills")), nome: "forno" },
    { ref: doc(collection(db, "skills")), nome: "pasticceria" },
    { ref: doc(collection(db, "skills")), nome: "aperitivo" },
  ];
  skills.forEach((s) => batch.set(s.ref, { nome: s.nome }));
  const [sCassa, sForno, sPasticceria, sAperitivo] = skills.map((s) => s.ref.id);

  const dipendenti = [
    { ref: doc(collection(db, "dipendenti")), nome: "Mario", telefono: "", skillIds: [sCassa, sForno], tettoOre: 30, oreMaxGiorno: 8, minGiorniLiberi: 1, assenze: [] },
    { ref: doc(collection(db, "dipendenti")), nome: "Anna", telefono: "", skillIds: [sCassa, sPasticceria], tettoOre: 24, oreMaxGiorno: 6, minGiorniLiberi: 2, assenze: [] },
    { ref: doc(collection(db, "dipendenti")), nome: "Luca", telefono: "", skillIds: [sForno], tettoOre: 40, oreMaxGiorno: 8, minGiorniLiberi: 1, assenze: [] },
    { ref: doc(collection(db, "dipendenti")), nome: "Giulia", telefono: "", skillIds: [sCassa, sAperitivo], tettoOre: 20, oreMaxGiorno: 5, minGiorniLiberi: 3, assenze: [] },
    { ref: doc(collection(db, "dipendenti")), nome: "Sara", telefono: "", skillIds: [sPasticceria, sAperitivo], tettoOre: 28, oreMaxGiorno: 7, minGiorniLiberi: 2, assenze: [] },
  ];
  ...
  fasce.forEach((f) => batch.set(doc(collection(db, "fabbisogno")), f));
